...
const SupIncidentsList = () => {
  ...
  const isOpenView         = initFilter === "open";
  const isResolvedWeekView = initFilter === "resolved-week";
  ...
  const stats = useMemo(() => ({
    ...
    overdue:  incidents.filter(isOverdueFn).length,
  ...
  const filteredIncidents = useMemo(() => {
    ...
    if (isOverdueView)  list = list.filter(isOverdueFn);
    if (isOpenView)     list = list.filter((i) => i.status?.toLowerCase() === "open");
    if (isResolvedView) list = list.filter((i) => i.status?.toLowerCase() === "closed");
    if (isResolvedWeekView) {
      const weekAgo = new Date();
      weekAgo.setDate(weekAgo.getDate() - 7);
      list = list.filter((i) =>
        ["resolved", "closed"].includes(i.status?.toLowerCase()) &&
        new Date(i.updated_at || i.resolved_at || i.created_at) >= weekAgo
      );
    }
    ...
    if (search)
      list = list.filter((i) =>
        ...
        (i.assignedEngineer?.name ?? "").toLowerCase().includes(search.toLowerCase())
      );
    ...
    return list;
      ...
      isOverdueView, isResolvedView, isOpenView, isResolvedWeekView]);
  ...
  const pageSubtitle = isOverdueView
    ...
    : isResolvedWeekView
    ? "Incidents resolved within the last 7 days"
    : isResolvedView
  ...
  return (
    <Box sx={{ minHeight: "100vh", background: "#f0f2f5", position: "relative" }}>
      ...
      <Box sx={{ p: 4, maxWidth: "1200px", margin: "0 auto", mt: "54px", zIndex: 1 }}>
        <StyledPaper>
          ...
          <Box sx={{ mb: 3, display: "flex", alignItems: "center", gap: 2, flexWrap: "wrap" }}>
            ...
            {isOverdueView && (
              <Chip label="Overdue only" size="small" onDelete={() => navigate("/support/my-incident")}
                sx={{ height: 24, fontSize: 12, bgcolor: "#fef2f2", color: "#b91c1c", fontWeight: 600 }} />
            )}
            {isResolvedView && (
              <Chip label="Closed only" size="small" onDelete={() => navigate("/support/my-incident")}
                sx={{ height: 24, fontSize: 12, bgcolor: "#dcfce7", color: "#166534", fontWeight: 600 }} />
            )}
            {isOpenView && (
              <Chip label="Open only" size="small" onDelete={() => navigate("/support/my-incident")}
                sx={{ height: 24, fontSize: 12, bgcolor: "#fef3c7", color: "#92400e", fontWeight: 600 }} />
            )}
            {isResolvedWeekView && (
              <Chip label="This week" size="small" onDelete={() => navigate("/support/my-incident")}
                sx={{ height: 24, fontSize: 12, bgcolor: "#e0f2fe", color: "#0369a1", fontWeight: 600 }} />
            )}
          </Box>
                ...
                <TableBody>
                  {filteredIncidents
                    ...
                    .map((incident) => {
                      ...
                      const needsVerif = incident.status?.toLowerCase() === "resolved";
                      ...
                      return (
                        <TableRow
                          ...
                          sx={{
                            ...
                            backgroundColor: needsVerif ? alpha("#fef2f2", 0.3) : "inherit",
                          }}
                        >
                          {/* ID — Small Text (12px) */}
                          <TableCell>
                          ...
                          {/* Title — Small Text (12px) for dense table data */}
                          <TableCell>
                            <Tooltip title={incident.title}>
                              ...
                              >
                                {incident.title}
                                {needsVerif && (
                                  <Tooltip title="Awaiting your verification">
                          ...
                          <TableCell align="center">
                            <Tooltip title="View Details">
                              <IconButton
                                size="small"
                                onClick={() => navigate(`/support/incidents/${incident.id}`)}
                              >
...
export default SupIncidentsList;