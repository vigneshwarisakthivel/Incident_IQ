...
const ReadOnlyField = ({ label, value, multiline = false, rows = 1 }) => (
  <TextField
    ...
    sx={{
      "& .MuiInputBase-root": { backgroundColor: "#f9fafb", color: "#374151" },
...
const EngineerKnowledgeBase = () => {
  ...
  const handleSaveArticle = async () => {
    ...
    if (highestMatch >= 90) {
      setAlert({ open: true, message: "A very similar article already exists. Please review it instead.", severity: "error" });
      return;
    }
    try {
    ...
    <Box sx={{ minHeight: "100vh", background: "#f0f2f5", position: "relative", overflow: "hidden" }}>
        ...
        <StyledPaper>
          ...
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3, pb: 2, borderBottom: "1px solid #e5e7eb" }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              ...
              <Box>
                ...
                {/* Secondary Text → 14px */}
                <Typography fontSize={14} color="text.secondary">Browse knowledge articles to resolve incidents quickly</Typography>
              </Box>
            </Box>
            {/* Small Text → 12px for chip */}
            <Chip label={`${filteredArticles.length} Articles`} sx={{ bgcolor: "#f3f4f6", fontSize: "12px", fontWeight: 500 }} />
          </Box>
          ...
          <Box sx={{ display: "flex", gap: 2, mb: 3, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <FormControl size="small" sx={{ minWidth: 140 }}>
              ...
              <Select value={statusFilter} label="Status" onChange={(e) => setStatusFilter(e.target.value)} sx={{ height: 36, fontSize: "14px" }}>
            ...
            <FormControl size="small" sx={{ minWidth: 160 }}>
              ...
              <Select value={categoryFilter} label="Category" onChange={(e) => setCategoryFilter(e.target.value)} sx={{ height: 36, fontSize: "14px" }}>
                <MenuItem value="ALL" sx={{ fontSize: "14px" }}>All</MenuItem>
                {categories.map((cat) => (
                  <MenuItem key={cat} value={cat} sx={{ fontSize: "14px" }}>{capitalize(cat)}</MenuItem>
                ))}
              </Select>
            </FormControl>
          </Box>
          ...
          <Box sx={{ mb: 3, display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(3, 1fr)" }, gap: 2 }}>
            ...
            <Card sx={{ bgcolor: alpha("#10b981", 0.05), border: "1px solid", borderColor: alpha("#10b981", 0.2), minHeight: 70 }}>
              <CardContent sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", p: 1.5 }}>
                <Box sx={{ position: "relative", display: "inline-flex", mr: 1 }}>
                  <CircularProgress variant="determinate" value={(articles.filter((a) => a.status === "published").length / (articles.length || 1)) * 100} size={60} thickness={5} sx={{ color: "#10b981" }} />
                  <Box sx={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    {/* Normal Text → 16px for stat number */}
                    <Typography fontSize={16} fontWeight={600}>{articles.filter((a) => a.status === "published").length}</Typography>
                  </Box>
                </Box>
            ...
            <Card sx={{ bgcolor: alpha("#f59e0b", 0.05), border: "1px solid", borderColor: alpha("#f59e0b", 0.2), minHeight: 70 }}>
              <CardContent sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", p: 1.5 }}>
                <Box sx={{ position: "relative", display: "inline-flex", mr: 1 }}>
                  <CircularProgress variant="determinate" value={(articles.filter((a) => a.auto_resolve === true).length / (articles.length || 1)) * 100} size={60} thickness={5} sx={{ color: "#f59e0b" }} />
                  <Box sx={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Typography fontSize={16} fontWeight={600}>{articles.filter((a) => a.auto_resolve === true).length}</Typography>
                  </Box>
                </Box>
                <Box sx={{ textAlign: "right", flex: 1 }}>
            ...
            <Card sx={{ bgcolor: alpha("#3b82f6", 0.05), border: "1px solid", borderColor: alpha("#3b82f6", 0.2), minHeight: 70 }}>
              <CardContent sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", p: 1.5 }}>
                <Box sx={{ position: "relative", display: "inline-flex", mr: 1 }}>
                  <CircularProgress variant="determinate" value={(new Set(articles.map((a) => a.category)).size / (articles.length || 1)) * 100} size={60} thickness={5} sx={{ color: "#3b82f6" }} />
                  <Box sx={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Typography fontSize={16} fontWeight={600}>{new Set(articles.map((a) => a.category)).size}</Typography>
                  </Box>
                </Box>
                <Box sx={{ textAlign: "right", flex: 1 }}>
              ...
              <Table>
                <TableHead sx={{ backgroundColor: "#f9fafb" }}>
                  <TableRow>
                    ...
                    <TableCell sx={{ fontWeight: 600, fontSize: "14px", color: "#000000", width: "22%" }}>Title</TableCell>
                    <TableCell sx={{ fontWeight: 600, fontSize: "14px", color: "#000000", width: "12%" }}>Category</TableCell>
                    <TableCell sx={{ fontWeight: 600, fontSize: "14px", color: "#000000", width: "22%" }}>Incident Pattern</TableCell>
                    <TableCell sx={{ fontWeight: 600, fontSize: "14px", color: "#000000", width: "12%" }}>Status</TableCell>
                    <TableCell sx={{ fontWeight: 600, fontSize: "14px", color: "#000000", width: "12%" }}>Auto-Resolve</TableCell>
                ...
                <TableBody>
                  {filteredArticles
                    ...
                    .map((article) => (
                      <TableRow key={article.id} sx={{ "&:hover": { backgroundColor: "#f8fafc" }, "& td": { py: 1 } }}>
                        <TableCell>
                          <Tooltip title={article.title}>
                            {/* Small Text → 12px for truncated table data */}
                            <Typography fontSize="14px" fontWeight={500} sx={{ maxWidth: 160, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                              {article.title}
                            </Typography>
                          </Tooltip>
                          ...
                          {isOwnDraft(article) && (
                            <Chip label="My Draft" size="small" sx={{ height: 18, fontSize: 12, mt: 0.5, bgcolor: "#fef9c3", color: "#92400e" }} />
                          )}
                          {isOwnPending(article) && (
                            <Chip label="Awaiting Approval" size="small" sx={{ height: 18, fontSize: 12, mt: 0.5, bgcolor: "#dbeafe", color: "#1d4ed8" }} />
                          )}
                        </TableCell>
                        ...
                        <TableCell>
                          <Tooltip title={article.incident_pattern}>
                            <Typography fontSize="14px" color="text.secondary" sx={{ maxWidth: 160, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                        ...
                        <TableCell>
                          <Box sx={{
                            ...
                            bgcolor: article.status === "published" ? "#ecfdf5" : article.status === "draft" ? "#fef3c7" : "#eff6ff",
                            color: article.status === "published" ? "#059669" : article.status === "draft" ? "#d97706" : "#2563eb",
                          }}>
      ...
      >
          ...
          {viewArticle && (
            <Box sx={{ display: "flex", alignItems: "center", gap: 2, flexWrap: "wrap", ml: 4 }}>
              ...
              {viewArticle.auto_resolve ? (
                <Chip icon={<AutoFixHighIcon />} label="Auto Resolve" size="small" color="success" sx={{ height: 22, fontSize: 12 }} />
              ) : (
                <Chip label="Manual Resolution" size="small" variant="outlined" sx={{ height: 22, fontSize: 12 }} />
              )}
            </Box>
          )}
        </DialogTitle>
        ...
        <DialogContent sx={{ p: 3, display: "flex", flexDirection: "column", gap: 3 }}>
          {viewArticle && (
            <>
              {/* Incident Pattern — Secondary Text → 14px */}
              ...
              {viewArticle.tags?.length > 0 && (
                <Box sx={{ p: 2, borderRadius: 2, border: "1px solid #e5e7eb", bgcolor: "#f9fafb" }}>
                  <Typography fontWeight={600} fontSize={14} mb={1}>Tags</Typography>
...
export default EngineerKnowledgeBase;